    ...
    async login(req, res) {
        ...
        try {
            ...
            if (!isMatch) {
                ...
                return res.status(500).json({
                    status: false,
                    message: "Sorry, The credentials you have entered are incorrect, Please check and try again.",
                });
            }
            if (user.status!='Active') {
                return res.status(500).json({
                    status: false,
                    message:"Please contact with your account admin as your account is presently locked",
                });
            }
            if (user.attempt >= 10) {
                return res.status(500).json({
                    status: false,
                    message: "Maximum attempt limit exhausted. Please contact Your system admin",
                });
            }
            user.attempt = 0;
            
            var accountLog = new AccountLog();
            await accountLog.saveLog("saved", user, user);
            await user.save();
            const payload = {
                user: {
                    id: user.id,
